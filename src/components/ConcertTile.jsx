...
export default function ConcertTile({ concertId }) {
  const concert = concertsData.find((c) => c.id === concertId);
  if (!concert) return null;

  return (
    <div className="concert-container bg-white px-5 py-4 mt-4 rounded-lg shadow-sm hover:shadow-md transition-all duration-300 border-l-3 border-teal-600 text-left">
      <h5 className="concert-title text-lg font-semibold text-blue-800 pb-4">{concert.concertTitle}</h5>
      <div className="event-date-venue-container flex flex-col gap-2 mb-4 opacity-75 text-sm">
        {concert.dates.map((date, index) => {
          ...
          return (
            <div className="event-date-time-container flex items-start flex-col text-gray-600 mb-2" key={index}>
        ...
        {concert.program.map((work, index) => {
          return (
            <React.Fragment key={work}>
              ...
              <div className="concert-program-work-title w-full pt-0 text-gray-700">{worksData[work].workName}</div>
            </React.Fragment>
          );
        })}
      </div>
