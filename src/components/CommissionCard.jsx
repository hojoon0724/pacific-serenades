export default function CommissionCard({ work }) {
  return (
    <div className="work-container m-4 flex flex-col justify-between rounded-xl shadow-md overflow-hidden transition-all duration-300 hover:shadow-lg hover:translate-y-[-2px] bg-white">
      <div className="top-half">
        ...
        <div className="work-details p-4">
          <h2 className="work-title text-2xl font-bold text-gray-800">
            {work.workName} <span className="text-gray-600">({work.workYear})</span>
          </h2>
          {work.instrumentation && (
            <div className="work-instrumentation text-gray-700 mb-3 text-sm">{work.instrumentation}</div>
          )}

          {work.commissionedBy && (
            <div className="work-commissioned text-gray-600 italic mb-8 text-sm">{work.commissionedBy}</div>
          )}
        </div>
      </div>
